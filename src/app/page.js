      ...
      <section id="events" className="py-24 relative">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            ...
            <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm font-black uppercase tracking-widest text-[#0b87bd] hover:gap-4 transition-all group">
          ...
          {!events || events.length === 0 ? (
            <div className="bg-white p-20 text-center rounded-[3rem] border border-gray-100 shadow-sm relative overflow-hidden group">
              <div className="absolute top-0 left-0 w-full h-1 bg-[#0b87bd]/10"></div>
              ...
              <p className="text-gray-500 max-w-sm mx-auto font-medium leading-relaxed font-sans underline decoration-[#0b87bd]/20 underline-offset-4">
              ...
              {events.map((event) => (
                <div key={event.id} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-2xl hover:shadow-black/5 transition-all duration-500 group relative flex flex-col items-start h-full">
                  <div className="flex items-center justify-between w-full mb-8">
                    <div className="px-4 py-1.5 bg-[#f5f7f9] border border-gray-100 text-[#0b87bd] text-[10px] font-black rounded-full uppercase tracking-widest">
                      {event.clubs?.name}
                    </div>
                  ...
                  <h3 className="text-2xl font-black mb-4 text-[#1E1E2D] group-hover:text-[#0b87bd] transition-colors leading-tight">
                    {event.title}
                  </h3>
                  ...
                  <div className="mt-auto w-full pt-6 border-t border-gray-50 flex items-center justify-between">
                    ...
                    <Link href={`/dashboard/events/${event.id}`} className="w-10 h-10 bg-[#f5f7f9] rounded-xl flex items-center justify-center text-[#1E1E2D] hover:bg-[#0b87bd] hover:text-white transition-all">
