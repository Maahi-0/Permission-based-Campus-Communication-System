...
export default async function MembersPage() {
    ...
    // DEBUG: Try fetching just profiles first to see if RLS works
    ...
    if (membersError) {
        console.error('CRITICAL ERROR: Failed to fetch profiles table. RLS might be blocking or table does not exist.', membersError)
    } else {
        console.log('SUCCESS: Fetched profiles count:', members?.length)
    }
    ...
    console.log('Fetched members count:', members?.length || 0)
    console.log('Current user role:', profile?.role)

    const isAdmin = profile?.role === 'admin'
    const isLead = profile?.role === 'club_lead'

    return (
        <div className="pb-12 text-[#1E1E2D]">
            <Header
                title="Campus Directory"
                subtitle={isAdmin ? "Complete member registry with administrative controls." : "Browse all registered campus members."}
                user={user}
            />
                            ...
                            <div className="pt-4 border-t border-gray-50 flex items-center justify-between">
                                ...
                                {isAdmin && (
                                    <form action={async () => {
                                        ...
                                        await s.from('club_members').delete().eq('user_id', member.id)
                                        // Delete profile
                                        ...
                                        <button className="px-3 py-1.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all">
                                            Remove
                                        </button>
                                    </form>
                                )}
